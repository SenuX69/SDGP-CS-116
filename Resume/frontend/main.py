...
SECTIONS = {
    ...
    "skills":         ["skills", "technical skills", "core competencies", "technologies", "key skills", "competencies", "technical capability"],
    "projects":       ["projects", "personal projects", "key projects", "portfolio", "implemented projects", "academic projects"],
    ...
    "references":     ["references", "referees", "reference", "recommendations", "recommendation"],
...
def extract_name(lines: list[str]) -> str:
    for line in lines[:10]:
        ...
        if (
            ...
            and not any(lower_s.startswith(kw) for kws in SECTIONS.values() for kw in kws)
            and not any(c.isdigit() for c in s)
            and "chatbot" not in lower_s
            and "project" not in lower_s
            and "github" not in lower_s
            and "linkedin" not in lower_s
            and "student" not in lower_s
            and "undergraduate" not in lower_s
        ):
            return s
    return ""
